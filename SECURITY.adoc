...
ECHIDNA is in active development. The current major version (`+2.x+`) on
`+main+` receives security updates; pre-`+1.0+` lineages are not
supported. The on-disk version lives in link:Cargo.toml[`+Cargo.toml+`]
and link:CHANGELOG.adoc[`+CHANGELOG.adoc+`]; this file deliberately does not
pin a version number to avoid drift.
...
We take the security of ECHIDNA seriously. If you believe you have found
a security vulnerability, please report it to us as described below.

==== Where to Report

*DO NOT* report security vulnerabilities through public GitHub issues,
GitLab issues, the issue tracker, or any other public channel.
...
. *Assessment*: Team assesses severity and impact
. *Development*: Patch is developed and tested
. *Notification*: Security advisory prepared
. *Release*: Patch released with advisory
. *Recognition*: Reporter credited (if desired)
...
. *Review Configs*: Audit security-related configurations
...
. *Cryptography*: Use well-tested cryptographic libraries
...
We use the CVSS (Common Vulnerability Scoring System) v3.1 to assess
severity:
...
* *Core libraries*: Rust core, Julia ML layer, Idris2 ABI, Zig FFI
bridge, AffineScript / AffineScript UI components
* *Prover backends*: every `+ProverKind+` variant wired through the
dispatch pipeline — Tier-1 core through Tier-3 niche. The live
membership list is `+ProverKind::all()+` in `+src/rust/provers/mod.rs+`;
the human-readable mirror is
link:docs/PROVER_COUNT.adoc[`+docs/PROVER_COUNT.adoc+`].
* *Trust-hardening pipeline*: solver integrity, certificate checking,
axiom tracking, sandboxing, mutation testing, Pareto ranking, Bayesian
confidence — wired in `+src/rust/dispatch.rs+`.
* *Build system*: Justfile, CI/CD workflows under `+.github/workflows/+`
* *Containers*: `+Containerfile+`,
`+.containerization/Containerfile.wave3+`, Podman configurations
...
* *PGP key*: `+4A03639C1EB1F86C7F0C97A91835A14A2867091E+` — fetch via
`+gpg --recv-keys 4A03639C1EB1F86C7F0C97A91835A14A2867091E+` or from
https://keys.openpgp.org/search?q=j.d.a.jewell@open.ac.uk
* *GitHub*: https://github.com/hyperpolymath[@hyperpolymath]

=== Acknowledgments

We thank the following researchers and security professionals for
...
versioned alongside the project (see
link:CHANGELOG.adoc[`+CHANGELOG.adoc+`]). This file is intentionally
count-free and version-free in prose to avoid R5b drift against
link:Cargo.toml[`+Cargo.toml+`].
