...
* Backend tiering, count semantics (variants vs. impl files vs.
  ...
  in link:docs/PROVER_COUNT.adoc[`docs/PROVER_COUNT.adoc`].
...
`CHANGELOG.adoc` carry the live numbers.
____
...
| **AffineScript-TEA / HTML** | `src/ui/tea/` — TEA sources (compile pipeline unavailable); `src/ui/public/` — working static shell
|===
...
| `src/rust/provers/`        | `ProverKind` enum + per-backend `ProverBackend` impls (see link:docs/PROVER_COUNT.adoc[`docs/PROVER_COUNT.adoc`] for the live tier table)
...
| `src/rust/corpus/`         | 17 corpus adapters (see link:docs/CORPUS-ADAPTERS.adoc[`docs/CORPUS-ADAPTERS.adoc`])
...
| `docs/architecture/VERISIM-ER-SCHEMA.adoc` | Formal E-R schema — shared data dictionary echidna ↔ long-term memory store
...
| `src/ui/tea/`, `src/ui/public/` | TEA sources (compile pipeline unavailable) and working static shell
