    ...
    pub fn new() -> Self {
        ...
        patterns.insert(
            ...
            vec![
                ...
                DangerousPattern {
                    pattern: "unsafeCoerce".to_string(),
                    danger_level: DangerLevel::Reject,
                    explanation: "UNSOUND: unsafeCoerce bypasses the type system entirely"
                        .to_string(),
                },
            ],
        );
            ...
            for (line_num, line) in content.lines().enumerate() {
                for pattern in prover_patterns {
                    if line.contains(&pattern.pattern) {
                        ...
                        // Skip scaffold sorrys that echidna itself emits into
                        // generated proof output. These carry a marker comment
...
mod tests {
    ...
    #[test]
    fn test_lean_scaffold_sorry_skipped() {
        // Sorry lines carrying the ECHIDNA_SCAFFOLD_SORRY marker are
        // generated by echidna's own prover output and must NOT be flagged
        // as hand-written proof holes.
        let tracker = AxiomTracker::new();
        let content =
            "theorem _echidna_goal : 1 = 1 := by\n  simp\n  sorry -- ECHIDNA_SCAFFOLD_SORRY";
        let usages = tracker.scan(ProverKind::Lean, content);

        assert_eq!(
            usages.len(),
            0,
            "scaffold sorry must be skipped, got: {:?}",
            usages
        );
    }

    #[test]
    fn test_isabelle_scaffold_sorry_skipped() {
        let tracker = AxiomTracker::new();
        let content = "lemma foo:\n  \"1 = 1\"\n  sorry (* ECHIDNA_SCAFFOLD_SORRY *)";
        let usages = tracker.scan(ProverKind::Isabelle, content);

        assert_eq!(usages.len(), 0, "scaffold sorry must be skipped");
    }
