...
//! CSI (Confluence and Termination Solver) backend
//!
//! CSI is an automated solver for confluence and termination properties of
//! term rewriting systems (TRS). It outputs Certification Problem Format (CPF)
//! certificates which enable independent verification of results.
//!
//! Features:
//! - Confluence checking for TRS/CTRS
//! - Termination analysis for TRS/CTRS
//! - CPF certificate output for independent verification
//! - Support for conditional rewriting
...
/// CSI confluence/termination solver backend
...
#[async_trait]
impl ProverBackend for CSIBackend {
    ...
    async fn version(&self) -> Result<String> {
        let output = Command::new(&self.config.executable)
            ...
            .output()
            .await
            .context("Failed to run csi --version")?;
    ...
    async fn apply_tactic(&self, _state: &ProofState, _tactic: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!(
            "CSI is a fully automated solver - interactive tactics not supported"
        ))
    }
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let mut child = Command::new(&self.config.executable)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("Failed to spawn csi")?;

        {
            let stdin = child
                .stdin
                .as_mut()
                .ok_or_else(|| anyhow::anyhow!("Failed to open csi stdin"))?;
            stdin
                .write_all(trs.as_bytes())
                .await
                .context("Failed to write to csi stdin")?;
        }

        let output = child
            .wait_with_output()
            .await
            .context("Failed to wait for csi")?;
...
mod tests {
    ...
    fn test_csi_kind() {
        ...
        let backend = CSIBackend::new(config);
        assert_eq!(backend.kind(), ProverKind::CSI);
    }
