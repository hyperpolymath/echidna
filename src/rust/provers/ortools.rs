...
impl ORToolsBackend {
    ...
    fn parse_result(&self, output: &str) -> Result<bool> {
        ...
        if l.contains("infeasible") || l.contains("error") {
            Ok(false)
        } else if l.contains("optimal") || l.contains("feasible") || l.contains("solution") {
            Ok(true)
        } else {
            Err(anyhow::anyhow!("OR-Tools inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for ORToolsBackend {
    ...
    async fn apply_tactic(&self, _: &ProofState, _: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!(
            "OR-Tools: optimization solver, not interactive"
        ))
    }
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let mut child = Command::new(&self.config.executable)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("Failed to spawn OR-Tools")?;
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_ortools_export() {
        let b = ORToolsBackend::new(ProverConfig {
            executable: PathBuf::from("ortools_solve"),
            timeout: 10,
            ..Default::default()
        });
        ...
        s.goals.push(Goal {
            id: "g".to_string(),
            target: Term::Const("minimize".to_string()),
            hypotheses: vec![],
        });
        assert!(b.to_input_format(&s).unwrap().contains("goal"));
    }
    #[tokio::test]
    async fn test_ortools_parse() {
        let b = ORToolsBackend::new(ProverConfig {
            executable: PathBuf::from("ortools_solve"),
            timeout: 10,
            ..Default::default()
        });
        let s = b.parse_string("minimize x").await.unwrap();
        assert!(!s.goals.is_empty());
    }
    #[test]
    fn test_ortools_result() {
        let b = ORToolsBackend::new(ProverConfig {
            executable: PathBuf::from("ortools_solve"),
            timeout: 10,
            ..Default::default()
        });
        assert!(b.parse_result("Optimal solution found").unwrap());
        assert!(!b.parse_result("Problem is infeasible").unwrap());
    }
}
