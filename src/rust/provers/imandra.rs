...
pub struct ImandraBackend {
    config: ProverConfig,
}
impl ImandraBackend {
    pub fn new(config: ProverConfig) -> Self {
        ImandraBackend { config }
    }
    fn to_input_format(&self, state: &ProofState) -> Result<String> {
        let mut s = String::new();
        if let Some(g) = state.goals.first() {
            s.push_str(&format!("verify (fun () -> {})\n", g.target));
        }
        Ok(s)
    }
    fn parse_result(&self, output: &str) -> Result<bool> {
        let l = output.to_lowercase();
        if l.contains("proved") || l.contains("true") || l.contains("verified") {
            Ok(true)
        } else if l.contains("false") || l.contains("counterexample") || l.contains("error") {
            Ok(false)
        } else {
            Err(anyhow::anyhow!("Imandra inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for ImandraBackend {
    ...
    async fn apply_tactic(&self, _: &ProofState, _: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!("Imandra: use verify/instance"))
    }
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let mut child = Command::new(&self.config.executable)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("Failed to spawn Imandra")?;
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_imandra_export() {
        let b = ImandraBackend::new(ProverConfig {
            executable: PathBuf::from("imandra"),
            timeout: 10,
            ..Default::default()
        });
        ...
        assert!(b.to_input_format(&s).unwrap().contains("verify"));
    }
    #[tokio::test]
    async fn test_imandra_parse() {
        let b = ImandraBackend::new(ProverConfig {
            executable: PathBuf::from("imandra"),
            timeout: 10,
            ..Default::default()
        });
        let s = b.parse_string("let f x = x + 1").await.unwrap();
        assert!(!s.goals.is_empty());
    }
    #[test]
    fn test_imandra_result() {
        let b = ImandraBackend::new(ProverConfig {
            executable: PathBuf::from("imandra"),
            timeout: 10,
            ..Default::default()
