...
impl ProverBackend for SatallaxBackend {
    ...
    async fn apply_tactic(&self, _state: &ProofState, _tactic: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!(
            "Satallax is a fully automated prover - interactive tactics not supported"
        ))
    }
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin
                ...
                .await
                .context("Failed to write to Satallax stdin")?;
            stdin.flush().await?;
            drop(stdin);
        }
