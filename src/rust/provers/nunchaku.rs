...
pub struct NunchakuBackend { config: ProverConfig }
impl NunchakuBackend { pub fn new(c: ProverConfig) -> Self { Self { config: c } }
    fn binary(&self) -> PathBuf {
        if self.config.executable.as_os_str().is_empty() { PathBuf::from("nunchaku") }
        else { self.config.executable.clone() } } }

#[async_trait]
impl ProverBackend for NunchakuBackend {
    fn kind(&self) -> ProverKind { ProverKind::Nunchaku }
    ...
    async fn parse_file(&self, p: PathBuf) -> Result<ProofState> {
        let c = tokio::fs::read_to_string(&p).await.with_context(|| format!("Nunchaku: reading {}", p.display()))?;
        self.parse_string(&c).await }
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        let mut s = ProofState {
            goals: vec![Goal { id: "nunchaku-goal".into(), target: Term::Const(content.into()), hypotheses: vec![] }],
            context: ProofContext::default(), proof_script: vec![], metadata: Default::default() };
        s.metadata.insert("nunchaku_source".into(), serde_json::Value::String(content.into())); Ok(s) }
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        let src: String = state.metadata.get("nunchaku_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default();
        let dir = tempfile::Builder::new().prefix("echidna-nunchaku-").tempdir().context("Nunchaku: tempdir")?;
        let input = dir.path().join("check.nun");
        tokio::fs::write(&input, src.as_bytes()).await.context("Nunchaku: writing")?;
        ...
        match cmd.output().await {
            Ok(o) => {
                ...
                Ok(out.contains("SAT") || out.contains("counterexample") || out.contains("UNKNOWN"))
            }
            Err(e) => Err(anyhow!("Nunchaku: binary not runnable: {}", e)) } }
    async fn export(&self, s: &ProofState) -> Result<String> {
        Ok(s.metadata.get("nunchaku_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default()) }
...
#[cfg(test)]
mod tests { use super::*;
    #[test] fn kind_is_nunchaku() { assert_eq!(NunchakuBackend::new(ProverConfig::default()).kind(), ProverKind::Nunchaku); } }
