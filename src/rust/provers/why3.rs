...
impl Why3Backend {
    ...
    fn parse_result(&self, output: &str) -> Result<bool> {
        ...
        if l.contains("valid") || l.contains("proved") {
            Ok(true)
        } else if l.contains("unknown") || l.contains("timeout") || l.contains("error") {
            Ok(false)
        } else {
            Err(anyhow::anyhow!("Why3 inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for Why3Backend {
    ...
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        ...
        for line in content.lines() {
            ...
            if l.starts_with("goal ") {
                state.goals.push(Goal {
                    id: format!("goal_{}", state.goals.len()),
                    target: Term::Const(l.to_string()),
                    hypotheses: vec![],
                });
            } else if l.starts_with("axiom ") {
                state.context.axioms.push(l.to_string());
            }
        }
    ...
    async fn apply_tactic(&self, _: &ProofState, _: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!("Why3: use built-in transformations"))
    }
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let input = if let Some(src) = state.metadata.get("why3_source").and_then(|v| v.as_str()) {
            src.to_string()
        } else {
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_why3_export() {
        let b = Why3Backend::new(ProverConfig {
            executable: PathBuf::from("why3"),
            timeout: 10,
            ..Default::default()
        });
        ...
        assert!(b.to_input_format(&s).unwrap().contains("module Export"));
    }
    #[tokio::test]
    async fn test_why3_parse() {
        let b = Why3Backend::new(ProverConfig {
            executable: PathBuf::from("why3"),
            timeout: 10,
            ..Default::default()
        });
        let s = b.parse_string("goal g : true").await.unwrap();
    ...
    fn test_why3_result() {
        ...
        assert!(b.parse_result("Valid").unwrap());
        assert!(!b.parse_result("Unknown").unwrap());
    }
}
