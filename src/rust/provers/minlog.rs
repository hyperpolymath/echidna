...
pub struct MinlogBackend {
    config: ProverConfig,
}
impl MinlogBackend {
    pub fn new(config: ProverConfig) -> Self {
        MinlogBackend { config }
    }
    ...
    fn parse_result(&self, output: &str) -> Result<bool> {
        ...
        if l.contains("proved") || l.contains("ok") {
            Ok(true)
        } else if l.contains("error") {
            Ok(false)
        } else {
            Err(anyhow::anyhow!("Minlog inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for MinlogBackend {
    ...
    async fn apply_tactic(&self, _: &ProofState, _: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!("Minlog: use proof terms"))
    }
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let mut child = Command::new(&self.config.executable)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("Failed to spawn Minlog")?;
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_minlog_export() {
        let b = MinlogBackend::new(ProverConfig {
            executable: PathBuf::from("minlog"),
            timeout: 10,
            ..Default::default()
        });
