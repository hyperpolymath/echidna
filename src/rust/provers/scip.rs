...
impl SCIPBackend {
    ...
    fn parse_result(&self, output: &str) -> Result<bool> {
        ...
        if l.contains("infeasible") || l.contains("error") {
            ...
            Err(anyhow::anyhow!("SCIP inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for SCIPBackend {
    ...
    async fn apply_tactic(&self, _: &ProofState, _: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!("SCIP: not an interactive prover"))
    }
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let input = if let Some(src) = state.metadata.get("scip_source").and_then(|v| v.as_str()) {
            src.to_string()
        } else {
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_scip_export() {
        let b = SCIPBackend::new(ProverConfig {
            executable: PathBuf::from("scip"),
            timeout: 10,
            ..Default::default()
        });
