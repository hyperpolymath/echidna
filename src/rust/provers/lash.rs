...
impl ProverBackend for LashBackend {
    ...
    async fn apply_tactic(&self, _state: &ProofState, _tactic: &Tactic) -> Result<TacticResult> {
        Err(anyhow::anyhow!(
            "Lash is a fully automated prover - interactive tactics not supported"
        ))
    }

    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        let tptp_code =
            if let Some(src) = state.metadata.get("lash_source").and_then(|v| v.as_str()) {
                src.to_string()
            } else {
                self.to_tptp(state)?
            };
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin
                ...
                .await
                .context("Failed to write to Lash stdin")?;
            stdin.flush().await?;
            drop(stdin);
        }
