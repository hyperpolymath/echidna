...
impl ProverBackend for HolLightBackend {
    ...
    async fn suggest_tactics(&self, state: &ProofState, limit: usize) -> Result<Vec<Tactic>> {
        ...
        Ok(crate::provers::gnn_augment_tactics(&self.config, state, "hol_light", suggestions, limit).await)
    }
...
mod tests {
    ...
    #[test]
    fn test_hol_light_parser_basic() -> Result<()> {
        let content = r#"
            let ADD_SYM = prove
              (`!m n. m + n = n + m`,
               REWRITE_TAC[ADD_AC]);;
    ...
    #[test]
    fn test_term_conversion() -> Result<()> {
        let config = ProverConfig::default();
        let backend = HolLightBackend::new(config);
        ...
        let term = backend.hol_to_term(&hol_term)?;
        assert!(matches!(term, Term::Var(_)));
        Ok(())
    }

    #[test]
    fn test_tactic_to_hol() -> Result<()> {
        let config = ProverConfig::default();
        let backend = HolLightBackend::new(config);

        let tactic = Tactic::Reflexivity;
        let hol_tactic = backend.tactic_to_hol(&tactic)?;
    ...
    fn test_export_format() -> Result<()> {
        ...
        let exported = backend.export_to_hol(&state)?;
    ...
    #[tokio::test]
    async fn test_tactic_suggestions() -> Result<()> {
        let config = ProverConfig::default();
        let backend = HolLightBackend::new(config);
