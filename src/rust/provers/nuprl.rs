...
impl NuprlBackend {
    ...
    fn parse_result(&self, output: &str) -> Result<bool> {
        ...
        if l.contains("proved") || l.contains("qed") {
            ...
            Err(anyhow::anyhow!("Nuprl inconclusive"))
        }
    }
}
#[async_trait]
impl ProverBackend for NuprlBackend {
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let input = if let Some(src) = state.metadata.get("nuprl_source").and_then(|v| v.as_str()) {
            src.to_string()
        } else {
    ...
    async fn suggest_tactics(&self, state: &ProofState, limit: usize) -> Result<Vec<Tactic>> {
        ...
        Ok(crate::provers::gnn_augment_tactics(&self.config, state, "nuprl", tactics, limit).await)
    }
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn test_nuprl_export() {
        let b = NuprlBackend::new(ProverConfig {
            executable: PathBuf::from("nuprl"),
            timeout: 10,
            ..Default::default()
        });
    ...
    #[tokio::test]
    async fn test_nuprl_parse() {
        let b = NuprlBackend::new(ProverConfig {
            executable: PathBuf::from("nuprl"),
            timeout: 10,
            ..Default::default()
        });
        let s = b.parse_string("True").await.unwrap();
        assert!(!s.goals.is_empty());
    }
    #[test]
    fn test_nuprl_result() {
        let b = NuprlBackend::new(ProverConfig {
            executable: PathBuf::from("nuprl"),
            timeout: 10,
            ..Default::default()
