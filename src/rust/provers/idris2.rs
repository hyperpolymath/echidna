                ...
                if c == '?' {
                    ...
                    while let Some(&nc) = chars.peek() {
                        if nc.is_alphanumeric() || nc == '_' {
                            hole_name.push(chars.next().unwrap());
                        } else {
                            break;
                        }
                    }
...
impl ProverBackend for Idris2Backend {
    ...
    async fn export(&self, state: &ProofState) -> Result<String> {
        ...
        output.push_str("-- SPDX-License-Identifier: MIT OR Palimpsest-0.6\n\n");
...
mod tests {
    ...
    fn test_parse_import() {
        ...
        let (_, decl) = parse_import(input).unwrap();
        match decl {
    ...
    fn test_parse_pragma() {
        ...
        let (_, decl) = parse_pragma(input).unwrap();
        match decl {
            Idris2Decl::Pragma { directive, args } => {
    ...
    fn test_parse_type_sig() {
        ...
        let (_, decl) = parse_type_sig(input).unwrap();
        match decl {
            Idris2Decl::TypeSig { name, ty } => {
                assert_eq!(name, "foo");
    ...
    #[tokio::test]
    async fn test_extract_goals() {
        let config = ProverConfig::default();
        let backend = Idris2Backend::new(config);

        let content = "foo : Nat -> Nat\nfoo n = ?foo_impl\n";
        let goals = backend.extract_goals(content).await.unwrap();
