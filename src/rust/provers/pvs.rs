...
pub enum PVSExpr {
    ...
    RecordAccess { record: Box<PVSExpr>, field: String },
    /// Tuple expression: (e1, e2, ...)
    Tuple(Vec<PVSExpr>),
    /// Projection: e`n
    Projection { tuple: Box<PVSExpr>, index: usize },
...
pub enum PVSStrategy {
    ...
    LiftIf,
    /// Induction
    Induct { var: String, scheme: Option<String> },
    /// Expand definition
    Expand {
        names: Vec<String>,
        fnum: Option<i32>,
    ...
    Lemma(String),
    /// Use type predicates
    TypePred { exprs: Option<Vec<PVSExpr>> },
    /// Apply rewrite rules
    Rewrite {
        names: Vec<String>,
        fnum: Option<i32>,
    ...
    Copy(i32),
    /// Name expression
    Name { name: String, expr: PVSExpr },
    /// Postpone current goal
    Postpone,
    /// Undo last step
    Undo,
    /// Custom strategy
    Custom { name: String, args: Vec<PVSExpr> },
}
...
impl<'a> PVSParser<'a> {
    ...
    pub fn parse_theory(&mut self) -> Result<PVSTheory> {
        ...
        if self.peek_char() == Some('[') {
            ...
            while self.peek_char() != Some(']') {
                ...
                parameters.push(PVSFormalParameter {
                    name: param_name,
                    kind,
                });
    ...
    fn parse_declaration(&mut self) -> Result<Option<PVSDeclaration>> {
        ...
        if let Some(kind) = formula_kind {
            ...
            return Ok(Some(PVSDeclaration::FormulaDecl {
                name,
                kind,
                formula,
            }));
...
impl PVSBackend {
        ...
        match expr {
            ...
            PVSExpr::Lambda { bindings, body } => {
                ...
                for binding in bindings.iter().rev() {
                    term = Term::Lambda {
                        ...
                        param_type: Some(Box::new(Term::Const(Self::type_to_string(
                            &binding.var_type,
                        )))),
                        body: Box::new(term),
                    };
            ...
            PVSExpr::Record(fields) => {
                let field_terms: Vec<Term> = fields
                    .iter()
                    .flat_map(|(name, value)| {
                        vec![Term::Const(name.clone()), Self::expr_to_term(value)]
                    })
                    .collect();
                Term::App {
            ...
            PVSExpr::RecordAccess { record, field } => Term::App {
                func: Box::new(Term::Const(format!("field_{}", field))),
                args: vec![Self::expr_to_term(record)],
            },
            PVSExpr::Projection { tuple, index } => Term::App {
    ...
    fn type_to_string(t: &PVSType) -> String {
        match t {
            ...
            PVSType::Function { domain, range } => {
                format!(
                    "[{} -> {}]",
                    Self::type_to_string(domain),
                    Self::type_to_string(range)
                )
            },
    ...
    fn term_to_expr(term: &Term) -> PVSExpr {
        match term {
            ...
            Term::Sigma {
                ...
                // Sigma types map to EXISTS in PVS
                PVSExpr::Exists {
                    bindings: vec![PVSBinding {
                        name: param.clone(),
                        var_type: Box::new(Self::term_to_type(param_type)),
            ...
            Term::Match {
                ...
                let selections: Vec<PVSCaseSelection> = branches
                    .iter()
            ...
            Term::Hole(name) => PVSExpr::Name(if name.is_empty() {
                "_".to_string()
            } else {
                name.clone()
            }),
    ...
    fn format_strategy(strategy: &PVSStrategy) -> String {
        match strategy {
            ...
            PVSStrategy::Replace { fnum, direction } => match direction {
...
impl ProverBackend for PVSBackend {
    ...
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        ...
        for decl in &theory.declarations {
            match decl {
                PVSDeclaration::FormulaDecl {
                    ...
                    | PVSFormulaKind::Theorem
                    | PVSFormulaKind::Corollary
                    | PVSFormulaKind::Proposition
                    | PVSFormulaKind::Obligation
                    | PVSFormulaKind::Challenge
        ...
        Ok(ProofState {
            ...
            metadata: {
                ...
                meta.insert(
                    "prover".to_string(),
                    serde_json::Value::String("PVS".to_string()),
                );
                meta
            },
        })
    ...
    async fn suggest_tactics(&self, state: &ProofState, limit: usize) -> Result<Vec<Tactic>> {
        ...
        Ok(
            crate::provers::gnn_augment_tactics(&self.config, state, "PVS", suggestions, limit)
                .await,
        )
    }
    ...
    async fn search_theorems(&self, pattern: &str) -> Result<Vec<String>> {
        ...
        let matches: Vec<String> = common_lemmas
            .iter()
...
mod tests {
    ...
    fn test_format_expr() {
        let expr = PVSExpr::Forall {
            ...
            body: Box::new(PVSExpr::Application {
                function: Box::new(PVSExpr::Name(">=".to_string())),
                arguments: vec![PVSExpr::Name("x".to_string()), PVSExpr::Number(0)],
            }),
        };
    ...
    fn test_expr_to_term_roundtrip() {
        let expr = PVSExpr::Application {
            function: Box::new(PVSExpr::Name("+".to_string())),
            arguments: vec![PVSExpr::Name("x".to_string()), PVSExpr::Number(1)],
        };
