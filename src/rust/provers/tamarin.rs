...
impl ProverBackend for TamarinBackend {
    ...
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        let mut state = self.parse_spthy(content)?;
        state.metadata.insert(
            "tamarin_source".to_string(),
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        ...
        let spthy_code = if let Some(src) = state
            .metadata
            .get("tamarin_source")
            .and_then(|v| v.as_str())
        {
    ...
    async fn suggest_tactics(&self, state: &ProofState, limit: usize) -> Result<Vec<Tactic>> {
        ...
        Ok(
            crate::provers::gnn_augment_tactics(&self.config, state, "tamarin", tactics, limit)
                .await,
        )
    }
