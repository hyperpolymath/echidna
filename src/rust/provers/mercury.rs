...
pub struct MercuryBackend { config: ProverConfig }
impl MercuryBackend { pub fn new(c: ProverConfig) -> Self { Self { config: c } }
    fn binary(&self) -> PathBuf {
        if self.config.executable.as_os_str().is_empty() { PathBuf::from("mmc") }
        else { self.config.executable.clone() } } }

#[async_trait]
impl ProverBackend for MercuryBackend {
    fn kind(&self) -> ProverKind { ProverKind::Mercury }
    ...
    async fn parse_file(&self, p: PathBuf) -> Result<ProofState> {
        let c = tokio::fs::read_to_string(&p).await.with_context(|| format!("Mercury: reading {}", p.display()))?;
        self.parse_string(&c).await }
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        let mut s = ProofState {
            goals: vec![Goal { id: "mercury-file".into(), target: Term::Const(content.into()), hypotheses: vec![] }],
            context: ProofContext::default(), proof_script: vec![], metadata: Default::default() };
        s.metadata.insert("mercury_source".into(), serde_json::Value::String(content.into())); Ok(s) }
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        let src: String = state.metadata.get("mercury_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default();
        let dir = tempfile::Builder::new().prefix("echidna-mercury-").tempdir().context("Mercury: tempdir")?;
        let input = dir.path().join("check.m");
        tokio::fs::write(&input, src.as_bytes()).await.context("Mercury: writing")?;
        let mut cmd = Command::new(self.binary());
        cmd.arg("--check").arg(&input).stdout(Stdio::piped()).stderr(Stdio::piped());
    ...
    async fn export(&self, s: &ProofState) -> Result<String> {
        Ok(s.metadata.get("mercury_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default()) }
...
#[cfg(test)]
mod tests { use super::*;
    #[test] fn kind_is_mercury() { assert_eq!(MercuryBackend::new(ProverConfig::default()).kind(), ProverKind::Mercury); } }
