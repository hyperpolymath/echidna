...
pub struct NitpickBackend { config: ProverConfig }
impl NitpickBackend { pub fn new(c: ProverConfig) -> Self { Self { config: c } }
    fn binary(&self) -> PathBuf {
        if self.config.executable.as_os_str().is_empty() { PathBuf::from("isabelle") }
        else { self.config.executable.clone() } } }

#[async_trait]
impl ProverBackend for NitpickBackend {
    fn kind(&self) -> ProverKind { ProverKind::Nitpick }
    async fn version(&self) -> Result<String> {
        match Command::new(self.binary()).arg("version").output().await {
            Ok(o) if o.status.success() => Ok(format!("{}+nitpick", String::from_utf8_lossy(&o.stdout).trim())),
            Ok(_) => Ok("nitpick@unavailable".into()),
            Err(_) => Ok("nitpick@not-installed".into()) } }
    async fn parse_file(&self, p: PathBuf) -> Result<ProofState> {
        let c = tokio::fs::read_to_string(&p).await.with_context(|| format!("Nitpick: reading {}", p.display()))?;
        self.parse_string(&c).await }
    async fn parse_string(&self, content: &str) -> Result<ProofState> {
        let mut s = ProofState {
            goals: vec![Goal { id: "nitpick-theorem".into(), target: Term::Const(content.into()), hypotheses: vec![] }],
            context: ProofContext::default(), proof_script: vec![], metadata: Default::default() };
        s.metadata.insert("nitpick_source".into(), serde_json::Value::String(content.into())); Ok(s) }
    ...
    async fn verify_proof(&self, state: &ProofState) -> Result<bool> {
        let src: String = state.metadata.get("nitpick_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default();
        let dir = tempfile::Builder::new().prefix("echidna-nitpick-").tempdir().context("Nitpick: tempdir")?;
        let input = dir.path().join("Check.thy");
        tokio::fs::write(&input, src.as_bytes()).await.context("Nitpick: writing")?;
        let mut cmd = Command::new(self.binary());
        cmd.arg("process").arg("-e").arg("nitpick").arg(&input).stdout(Stdio::piped()).stderr(Stdio::piped());
    ...
    async fn export(&self, s: &ProofState) -> Result<String> {
        Ok(s.metadata.get("nitpick_source").and_then(|v| v.as_str()).map(String::from).unwrap_or_default()) }
...
#[cfg(test)]
mod tests { use super::*;
    #[test] fn kind_is_nitpick() { assert_eq!(NitpickBackend::new(ProverConfig::default()).kind(), ProverKind::Nitpick); } }
