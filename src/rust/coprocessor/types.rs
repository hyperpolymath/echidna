...
pub enum CoprocessorKind {
    ...
    // ── Phase 2b: subprocess CAS backends (ExternalSubprocess / Tier 1) ──

    /// PARI/GP number theory — subprocess to `gp`.  Advanced factoring (ECM /
    /// GNFS), multiplicative order, next-prime queries.  PARI is the gold
    /// standard for algorithmic number theory.
    PariGp,

    /// Maxima symbolic algebra — subprocess to `maxima`.  Simplification,
    /// polynomial factoring, symbolic differentiation.  Free CAS, GPL.
    Maxima,

    /// Singular polynomial algebra — subprocess to `Singular`.  Gröbner bases,
    /// ideal membership, Krull dimension over arbitrary fields.  GPL.
    Singular,

    /// GAP group theory — subprocess to `gap`.  Permutation group order,
    /// abelianness test, symmetric group order.  GPL.
    Gap,

    /// Macaulay2 algebraic geometry — subprocess to `M2`.  Gröbner bases,
    /// Krull dimension, degree of quotient rings over QQ.  GPL.
    Macaulay2,
}
...
pub enum CoprocessorOp {
    ...
    // ── PARI/GP: advanced number theory (subprocess) ──────────────────────
    /// Complete integer factorisation via PARI/GP (ECM/GNFS for large n).
    /// More powerful than the Math backend's Pollard-rho for large semi-primes.
    /// `n` must be a decimal integer string.
    PariGpFactor { n: String },

    /// Multiplicative order of `a` in (Z/nZ)*.  Returns `BigInt(ord)` when
    /// `gcd(a,n) = 1`, `Failure` otherwise.  Both strings are decimal integers.
    PariGpZnorder { a: String, n: String },

    /// Smallest prime strictly greater than `n`.  `n` is a decimal integer.
    PariGpNextPrime { n: String },

    // ── Maxima: symbolic algebra (subprocess) ────────────────────────────
    /// Symbolically simplify a Maxima expression.  `expr` uses Maxima syntax
    /// (e.g. `"x^2+2*x+1"`).  Returns the simplified form as `Text`.
    MaximaSimplify { expr: String },

    /// Factorise a Maxima polynomial expression over the rationals.
    /// `expr` uses Maxima syntax.  Returns factored form as `Text`.
    MaximaFactor { expr: String },

    /// Differentiate `expr` with respect to variable `var`.
    /// Both use Maxima syntax.  Returns the derivative as `Text`.
    MaximaDiff { expr: String, var: String },

    // ── Singular: polynomial algebra (subprocess) ─────────────────────────
    /// Compute a Gröbner basis of the ideal generated by `polys` over the
    /// ring k[vars] where k is GF(`ring_char`) if `ring_char > 0`, or QQ
    /// if `ring_char == 0`.  Uses graded-reverse-lex (`dp`) order.
    /// Returns the basis generators as `Text` (one polynomial per line).
    SingularGroebner { ring_char: u32, vars: Vec<String>, polys: Vec<String> },

    /// Test whether `poly` belongs to the ideal generated by `ideal` in
    /// k[vars].  Returns `Boolean(true)` if `poly` is in the ideal.
    SingularIdealMembership {
        ring_char: u32,
        vars: Vec<String>,
        poly: String,
        ideal: Vec<String>,
    ...
    /// Krull dimension of the quotient ring k[vars] / (polys).
    /// Returns `BigInt(dim)`.
    SingularDimension { ring_char: u32, vars: Vec<String>, polys: Vec<String> },

    // ── GAP: group theory (subprocess) ───────────────────────────────────
    /// Order of the permutation group generated by `generators`.
    /// Each generator is a permutation in cycle notation, e.g. `"(1,2,3)"`.
    /// Returns `BigInt(order)`.
    GapGroupOrder { generators: Vec<String> },

    /// Whether the permutation group generated by `generators` is abelian.
    /// Returns `Boolean(true/false)`.
    GapIsAbelian { generators: Vec<String> },

    /// Order of the symmetric group S_n (= n!).  Useful as a GAP smoke test.
    /// Returns `BigInt(n!)`.
    GapSymmetricGroupOrder { n: u32 },

    // ── Macaulay2: algebraic geometry (subprocess) ────────────────────────
    /// Gröbner basis of the ideal generated by `polys` in QQ[vars].
    /// Returns generators as `Text` (one polynomial per line).
    Macaulay2GroebnerBasis { vars: Vec<String>, polys: Vec<String> },

    /// Krull dimension of QQ[vars] / (polys).  Returns `BigInt(dim)`.
    Macaulay2Dimension { vars: Vec<String>, polys: Vec<String> },

    /// Degree of the quotient ring QQ[vars] / (polys).  For a
    /// zero-dimensional ideal this equals the number of points (with
    /// multiplicity) on the variety.  Returns `BigInt(degree)`.
    Macaulay2Degree { vars: Vec<String>, polys: Vec<String> },
}
...
pub enum CoprocessorOutcome {
    ...
    /// A plain text string — CAS symbolic output (PARI/GP, Maxima, Singular,
    /// GAP, Macaulay2).  Not hex-encoded; the content is human-readable CAS
    /// output (polynomial, group description, etc.).
    Text(String),
