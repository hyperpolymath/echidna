...
use echidna::diagnostics::{DegradationMode, HealthStatus};
...
impl DiagnosticsREPL {
    ...
    fn print_health(&self, health: &HealthStatus) {
        ...
        println!(
            "System Degraded: {}",
            if health.is_degraded() { "YES" } else { "NO" }
        );
        ...
        println!(
            "  Loaded: {}",
            if health.gnn_model_health.is_loaded {
                "YES"
            } else {
                "NO"
            }
        ...
        println!(
            "  nDCG: {:.4}",
            health.gnn_model_health.last_validation_nDCG
        );
        println!("  MRR: {:.4}", health.gnn_model_health.last_validation_MRR);
        println!(
            "  Meets threshold: {}",
            if health.gnn_model_health.nDCG_meets_threshold {
                "YES"
            } else {
                "NO"
            }
        ...
        println!(
            "  Fallback active: {}",
            if health.gnn_model_health.fallback_active {
                "YES"
            } else {
                "NO"
            }
        ...
        println!(
            "  Cache hit rate: {:.1}%",
            health.gnn_model_health.fallback_cache_hit_rate * 100.0
        );
    ...
    fn print_prover_detail(&self, health: &HealthStatus, name: &str) {
        match health.prover_health.get(name) {
            Some(prover) => {
                ...
                println!(
                    "Available: {}",
                    if prover.is_available { "YES" } else { "NO" }
                );
    ...
    fn print_gnn_status(&self, health: &HealthStatus) {
        ...
        println!(
            "  Cache size: {} goals",
            health.gnn_model_health.fallback_cache_size
        );
        println!(
            "  Cache hit rate: {:.1}%",
            health.gnn_model_health.fallback_cache_hit_rate * 100.0
        ...
        println!(
            "  Max latency: {:.1} ms",
            health.gnn_model_health.fallback_max_latency_ms
        );
        println!();
    }
    ...
    fn print_corpus_status(&self, health: &HealthStatus) {
        ...
        println!(
            "Size change: {:.1}%",
            health.corpus_health.size_change_percent
        );
