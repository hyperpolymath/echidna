...
This directory contains all interface implementations for ECHIDNA
...
`+suggestTactics(prover, context,   goalState)+` (ad-hoc, ML-backed)
...
** RPCs: SubmitProof, GetProofStatus, StreamProof, ApplyTactic,
CancelProof
...
** Endpoints: `+/api/v1/provers+`, `+/api/v1/proofs+`,
`+/api/v1/proofs/:id/tactics+`
...
All interfaces communicate with the ECHIDNA core via: - *Option 1:* FFI
(Foreign Function Interface) - direct Rust calls - *Option 2:* IPC
(Inter-Process Communication) - message passing - *Option 3:* Shared
