...
** {blank}
[loweralpha]
. have a default value (e.g. `+@N :Bool = false+`), OR
** {blank}
[loweralpha, start=2]
. be paired with a `+has<Name> :Bool+` sibling for optional semantics.
...
* Changing the stable `+ProverKind+` discriminant mapping (mirrored in
`+src/rust/ffi/mod.rs+`).

=== Deprecation flow

[arabic]
. Mark the field obsolete in this file and in a comment on the
`+.capnp+` line. Add a new replacement field at the next `+@N+`.
. Wait ONE minor version before removal candidacy (L1.0 → L1.1).
. Field removal = major bump (L1.x → L2.0). Must ship an
...
If no overlap: server returns `+HelloAck { agreedMajor = 0 }+` and
closes.

=== ProverKind discriminant governance

Appending a new variant to `+ProverKind+` (`+src/rust/provers/mod.rs+`)
is SAFE iff:
...
at `+src/rust/ffi/mod.rs+` (see `+kind_to_u8_injective+` / round-trip
