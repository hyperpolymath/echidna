...
the ECHIDNA project a harassment-free experience for everyone,
...
unprofessional levels of intimacy
...
* GitHub repository (code, issues, pull requests, wiki) and any mirror
(e.g. GitLab CI mirror)
...
be reported through either of the following channels:

* *GitHub Security Advisories* (preferred — private, encrypted,
auditable):
https://github.com/hyperpolymath/echidna/security/advisories/new Tag the
report as a conduct matter.
* *Email* (fallback): `+j.d.a.jewell@open.ac.uk+` — maintainer; PGP key
...
*Lineage*: Adapted from the
https://www.contributor-covenant.org/[Contributor Covenant] v2.1.
Revisions tracked via `+git log CODE_OF_CONDUCT.adoc+`. This file is
intentionally version-free and date-free in prose to avoid R5b drift.
