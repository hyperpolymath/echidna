link:LICENSE[image:https://img.shields.io/badge/License-AGPL--3.0--or--later-blue.svg[License:
AGPL-3.0-or-later]]
https://www.thegreenwebfoundation.org/green-web-check/?url=nesy-prover.dev[image:https://api.thegreenwebfoundation.org/greencheckimage/nesy-prover.dev[Green
Hosting]]
...
(`+.machine_readable/+`, manifests, OCI labels) under MPL-2.0,
deliberately weak-copyleft so it stays usable for interoperability and
standards work; *documentation* under CC-BY-SA-4.0; and
*`+echidna-playground/+`* under MPL-2.0. The combined work distributes
as AGPL-3.0-or-later — run a modified ECHIDNA as a network service and
you must offer your users the modified source. MPL §3.3 (Secondary
Licenses) is what makes the MPL parts compatible inside the AGPL work.
Full statement: link:NOTICE[`+NOTICE+`].
____

*E*xtensible *C*ognitive *H*ybrid *I*ntelligence for *D*eductive
*N*eural *A*ssistance
...
source of truth: link:docs/PROVER_COUNT.adoc[`+docs/PROVER_COUNT.adoc+`].
The REST API (`+GET+` `+/api/provers+`) exposes the Tier-1 _core_ set by
default; all other backends are reachable through `+ProverKind+` and the
dispatch pipeline. See
link:docs/SUPPORTED_PROVERS.adoc[`+docs/SUPPORTED_PROVERS.adoc+`] for the
...
combination, Pareto multi-objective frontier) see the Arbitration
section below.
* *Neurosymbolic architecture* — Julia ML layer suggests tactics; formal
provers always have the final word.
...
ever _accelerate_ a proof attempt — never substitute for one.
...
The Tier-1 _core_ set is what `+GET+` `+/api/provers+` exposes by
default and what is required to pass for green CI; the live membership
list is `+ProverKind::all_core()+` in `+src/rust/provers/mod.rs+`,
mirrored in human-readable form at
link:docs/SUPPORTED_PROVERS.adoc[`+docs/SUPPORTED_PROVERS.adoc+`] (with
required external binaries and install hints). The remaining tiers
(extended, niche, Wave-3 secured, Wave-2 modal/real-algebraic, pure-Rust
...
link:docs/PROVER_COUNT.adoc[`+docs/PROVER_COUNT.adoc+`].
...
disagreements for human review. Supports SMT, ATP, and ITP solver pools.
...
explicit opt-in). Auto-detection selects the strongest available option.
...
negate subterms, replace constants) to verify the pipeline catches them.
Computes a mutation score with a default 95% threshold.
...
kernel re-checking). See Wire Schema for the formal data contracts.
...
timeout estimation (mean + 2 sigma). Wilson score intervals for mutation
score confidence. Prover ranking by composite score (success rate,
timeout rate, speed). JSON serialisation for persistence.
...
shape (Idris2 / Agda case studies in link:src/chapel/[`+src/chapel/+`]).
* *Three API surfaces*:
** GraphQL (async-graphql, port 8081)
** gRPC (tonic + Protocol Buffers, port 50051)
** REST (axum + OpenAPI/Swagger, port 8000)
* *AffineScript / AffineScript UI*: AffineScript components are being
ported to AffineScript-TEA as the stdlib primitives needed for the port
...
Run `+just+` `+doctor+` to verify what’s actually installed; `+just+`
`+heal+` will offer to install missing pieces non-destructively.
...
See link:docs/SUPPORTED_PROVERS.adoc[`+docs/SUPPORTED_PROVERS.adoc+`] for
the Tier-1 core set, required binaries, and how to expose additional
backends through the API.
...
* *AffineScript-TEA*: UI sources in `src/ui/tea/` (compile pipeline unavailable); working static shell in `src/ui/public/`
...
....
src/rust/
  provers/          # Per-backend ProverBackend impls (see docs/PROVER_COUNT.adoc for the live tier table)
...
and exposes a `+pub+` `+fn+` `+ingest(root:+` `+&Path)+` `+→+`
`+Result<Corpus>+` returning the canonical `+Corpus+` struct defined at
...
|`+minif2f+` |`+*.lean+` / `+*.thy+` / `+*.ml+` / `+*.mm+` / `+*.v+`
|MiniF2F olympiad multi-target
|===
...
link:docs/CORPUS-ADAPTERS.adoc[`+docs/CORPUS-ADAPTERS.adoc+`].
...
link:data/synonyms/[`+data/synonyms/+`] and drive the `+echidna+`
`+suggest+` variant-tester (`+src/rust/suggest/synonyms.rs+`). The
schema (canonical + aliases + tactic_class + version range) and contract
are documented in
link:data/synonyms/README.adoc[`+data/synonyms/README.adoc+`].
...
|Simple agreement count across solvers; flags disagreements for human
review. Original cross-checking surface.
...
returns posterior `+(p_proven,+` `+p_refuted,+` `+p_unknown)+` with
Shannon entropy. Numerically stable; Timeout/Unknown/Error contribute LR
= 1.

|Dempster-Shafer
|link:src/rust/verification/dempster_shafer.rs[`+verification/dempster_shafer.rs+`]
|Mass-function combination with explicit ignorance; tracks the conflict
scalar `+K+`. Refuses to combine when total conflict > 0.95
(`+ArbiterError::ExcessiveConflict+`).

|Pareto frontier
|link:src/rust/verification/pareto_arbiter.rs[`+verification/pareto_arbiter.rs+`]
(multi-objective arbitration sibling to `+verification/pareto.rs+` — the
candidate-ranking pre-arbiter) |Multi-objective dominance across time /
trust / memory / proof-size; configurable `+Tiebreak+` policy chooses a
single recommended pick from the frontier.
|===
...
* link:docs/architecture/VERISIM-ER-SCHEMA.adoc[`+docs/architecture/VERISIM-ER-SCHEMA.adoc+`]
...
Replaces the aspirational text in the 2026-04-17 triangulation document.
...
link:CHANGELOG.adoc[`+CHANGELOG.adoc+`] carry the live numbers.

* Smoke E2E integration tests across the Tier-1 prover backends
* Property-based tests (PropTest)
* Criterion benchmarks across all critical paths
* Idris2 ABI type-check (`+idris2+` `+--build+`
`+src/abi/echidnaabi.ipkg+`) enforces zero `+believe_me+` /
`+assert_total+` / `+postulate+` in the ABI modules.
...
`+just+` `+--list+` enumerates every recipe; `+just+` `+help-me+` prints
an onboarding subset.

== Current Status

The authoritative status surface — release history, what shipped in each
minor, what’s targeted next — is link:CHANGELOG.adoc[`+CHANGELOG.adoc+`].
For roadmap items beyond the next release, see
link:docs/ROADMAP.adoc[`+docs/ROADMAP.adoc+`].
...
* *Estate migrations in flight*: AffineScript-TEA UI compilation; legacy JavaScript runtime→Bun
...
* link:docs/PROVER_COUNT.adoc[`+docs/PROVER_COUNT.adoc+`] — canonical
prover-count and tier table
* link:docs/ARCHITECTURE.adoc[`+docs/ARCHITECTURE.adoc+`] — system overview
* link:docs/CORPUS-ADAPTERS.adoc[`+docs/CORPUS-ADAPTERS.adoc+`] — 17-adapter
corpus ingest index
* link:docs/architecture/VERISIM-ER-SCHEMA.adoc[`+docs/architecture/VERISIM-ER-SCHEMA.adoc+`]
— VeriSim ↔ ECHIDNA entity-relationship schema
* link:docs/decisions/2026-06-01-saturation-campaign.adoc[`+docs/decisions/2026-06-01-saturation-campaign.adoc+`]
— ADR for the saturation campaign (corpora / vocab / synonyms / exchange
/ arbitration)
* link:docs/handover/PROVER-CORPUS-SATURATION-LANE.adoc[`+docs/handover/PROVER-CORPUS-SATURATION-LANE.adoc+`]
— saturation lane handover
* link:docs/ENV-VARS.adoc[`+docs/ENV-VARS.adoc+`] — environment variables
...
link:RSR_COMPLIANCE.adoc[`+RSR_COMPLIANCE.adoc+`] for the full hard-rule
...
This project is licensed under the link:LICENSE[GNU Affero General
Public License v3.0 or later]. Individual files may carry earlier
MPL-2.0 SPDX headers pending the header migration sweep.
...
link:CHANGELOG.adoc[`+CHANGELOG.adoc+`] and the git log; not duplicated here
to avoid drift.
