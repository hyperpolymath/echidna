...
image:https://img.shields.io/badge/Provers-128-green.svg[Provers: 128]
image:https://img.shields.io/badge/Tests-917%2B-brightgreen.svg[Tests: 917+]
...
A neurosymbolic theorem proving platform with 128 prover backends,
...
* **The only system with 128+ prover backends** (vs 1 for Isabelle, Coq, Lean, Z3, etc.)
...
* **The most comprehensive axiom tracking** (4 danger levels across 128 provers)
...
ECHIDNA orchestrates 128 theorem provers, SMT solvers, first-order ATPs, and
...
All 128 backends implement the `ProverBackend` trait: parse, verify, export,
...
----
src/rust/
  provers/          # 128 prover backend implementations (ProverBackend trait)
...
917+ library tests passing (`cargo test --lib --features verisim`, 2 ignored).

* 884 tests default build (`cargo test --lib`)
* 917 tests with VeriSim integration (`--features verisim`)
* Smoke E2E integration tests across all prover backends
