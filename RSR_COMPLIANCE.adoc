...
| RSR-H13 | Guix is the sole package manager; sealed-container escape only; guix deprecated estate-wide 2026-06-01 | `.claude/CLAUDE.md`
...
* Language tiers: Tier-1 = Rust, Elixir, Zig, Ada, Haskell, AffineScript/AffineScript; Tier-2 = Nickel, Racket, Guile. (Guix removed from Tier-2 on 2026-06-01 per estate-wide deprecation.)
* Coverage above 80 %.
...
2. *Polyglot source layout*: `src/` contains per-language subdirectories
   ...
   `src/ada/`, `src/affinescript/`, `src/ui/`, `src/interfaces/`) rather than
