...
mod gnn_training_integration_tests {
    use echidna::diagnostics::{load_training_metrics, update_health_with_metrics, HealthStatus};
    use std::path::Path;
    ...
    fn test_load_and_apply_training_metrics() {
        ...
        let metrics = load_training_metrics(metrics_path).expect("Failed to load training metrics");
        ...
        assert!(
            health_pct >= 20.0,
            "With GNN loaded at {:.2}, health should be >= 20%",
            metrics.nDCG
        );
