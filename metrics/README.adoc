...
the vocabulary horizon and triangulation capacity. Every metric writes
its result to VeriSimDB via `+POST /api/v1/metrics+`; the local JSONL
file is a cache, not the source of truth.
...
|`+alignment_rate.jl+` |RDF-triples linking named theorems across
provers |≥ 30 %
...
* Single loader in `+corpus_loader.jl+` — reads all
`+proof_states_*.jsonl+` once per run and hands per-prover slices to
each metric.
