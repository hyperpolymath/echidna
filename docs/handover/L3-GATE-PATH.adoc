...
`docs/handover/L3-LIVE-PROVER-CI-PROMPT.adoc` "Hand to L1 when":

* All four waves land or are explicitly deferred with rationale in
  `STATE.a2ml`.
* Tier-1 has run green on main for ≥7 days.

`docs/handover/TODO.adoc` adds two operational pre-requisites (P0, P1).
...
| ⚠ UNVERIFIED
| TODO.adoc P0 row: "Wave-2 installers (idris2 / isabelle / dafny /
...
* `docs/handover/L3-LIVE-PROVER-CI-PROMPT.adoc` — original L3 mission.
* `docs/handover/TODO.adoc` — P0 / P1 rows referenced above.
* `docs/handover/L1-CAPNPROTO-PROMPT.adoc` — the prompt that fires
  once this gate closes.
* `docs/handover/L2-CHAPEL-PROMPT.adoc` — blocked behind L1 + this gate.
