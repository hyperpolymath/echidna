...
Returns all 30 supported prover backends with tier and complexity
information.
...
*Path Parameters:* - `+kind+` — Prover identifier (e.g., `+coq+`,
`+lean+`, `+z3+`, `+isabelle+`)
...
Returns ML-suggested tactics for the current proof state. Uses Julia ML
service (port 8090) with fallback to prover built-in suggestions.
